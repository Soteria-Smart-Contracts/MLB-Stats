...
async function BackTestSeason(){
    ...
    for(let i = 0; i < json.length; i++){
        ...
        try{ PitcherTwoFIP = await parseFloat(await GetPitcherFIPfromName(json[i][i][1]));}
        catch(err){}
        ...
        let Over = json[i][i][3];
        let AverageFIP = await Math.max(PitcherOneFIP, PitcherTwoFIP);
        console.log('avgfip: ' + AverageFIP);
    ...
    console.log('TwoFiveUnder' + TwoFiveU);
    ...
    console.log('FourZero' + FourZero);
    console.log('FourOne' + FourOne);
    console.log('FourTwo' + FourTwo);
